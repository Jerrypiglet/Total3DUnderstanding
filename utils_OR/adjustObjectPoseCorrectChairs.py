...
def findSupport(lverts, boxes, cats ):
    ...
    return floorList, boxList, isForcedFloor
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out', default="../scenes/xml/", help="outDir of xml file" )
    ...
    # root_dir = Path('/newfoundland2/ruizhu/siggraphasia20dataset/code/Routine/DatasetCreation/')
    # params = [str(root_dir / Path(params[key])) for key in params]
    # print(params)
    ...
    with open('forcedFloor.txt', 'w') as fOut:
        fOut.write('#Forced floor scenes\n')
    ...
    for r in JSONHelper.read(filename_json ):
        ...
        floorList, boxList, isForcedFloor = findSupport(lverts, boxes, cats )

        if isForcedFloor == True:
            print('Chair, sofa or bench\'s poisition will be changed')
            ...
            if isBeyondRange == True:
                ...
                with open('forcedFloor.txt', 'a') as fOut:
                    fOut.write('%s\n' % id_scan )
                # Write new boxes
                ...
                with open(transformName, 'wb') as datOut:
                    pickle.dump(transformList, datOut )
