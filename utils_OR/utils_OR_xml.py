...
def parse_XML_for_shapes(root, root_uv_mapped):
    ...
    shapes_list = []
    for shape in shapes:
        shape_dict = {'id': shape.get('id')}
        # if 'scene' in shape_dict['id']:
        #     continue
    #     print(shape.get('string'))
        shape_dict[shape.findall('string')[0].get('name')] = shape.findall('string')[0].get('value')
    #     shape_dict[shape.findall('transform')[0].get('name')] = shape.findall('string')[0].get('value')
