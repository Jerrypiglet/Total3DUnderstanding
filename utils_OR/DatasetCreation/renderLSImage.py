...
if __name__ == '__main__':
    ...
    parser.add_argument('--program', default='/siggraphasia20dataset/OptixRendererLS/src/bin/optixRenderer' )
    # Start and end point
    ...
    for k in range(opt.rs, min(opt.re, len(scenes ) ) ):
        ...
        for n in range(0, len(lightList ) ):
            ...
            cmd0 = '%s --forceOutput -c cam.txt -f %s -o %s -m 0 --camStart 4 --camEnd 5 --maxIteration 5 --medianFilter ' \
                    % (opt.program, newFile, osp.join(outDir, 'imLS%d.rgbe' % n ) )
            ...
            DSNames = glob.glob(osp.join(outDir, 'imLS%d_*.rgbe' % n) )
