...
if __name__ == '__main__':
    ...
    parser.add_argument('--outRoot', default='/eccv20dataset/DatasetNew_test')
    parser.add_argument('--program', default='/siggraphasia20dataset/OptixRendererLSShading/src/bin/optixRenderer')
    parser.add_argument('--programNoOcclu', default='/siggraphasia20dataset/OptixRendererLSShadingNoOcclu/src/bin/optixRenderer')
    # Start and end point
    ...
    for k in range(opt.rs, min(opt.re, len(scenes ) ) ):
        ...
        for n in range(0, len(lightList ) ):
            ...
            sensor = newRoot.findall('sensor')[0]
            ...
            cmd0 = '%s -c cam.txt -f %s -o %s -m 0 --maxIteration 5 ' \
                    % (opt.program, newFile, osp.join(outDir, 'imDS%d.rgbe' % n ) )
            cmd1 = '--center %.4f %.4f %.4f ' % (center[0], center[1], center[2] )
            cmd2 = '--yAxis %.4f %.4f %.4f ' % (yAxis[0], yAxis[1], yAxis[2] )
            cmd3 = '--xAxis %.4f %.4f %.4f ' % (xAxis[0], xAxis[1], xAxis[2] )
            cmd4 = '--zAxis %.4f %.4f %.4f' % (zAxis[0], zAxis[1], zAxis[2] )
            ...
            DSNames = glob.glob(osp.join(outDir, 'imDS%d_*.rgbe' % n) )
            for DSName in DSNames:
                imId = DSName.split('/')[-1].split('_')[-1].split('.')[0]
                dstDir = osp.join(outDir, 'light_%s' % imId )
                if osp.isdir(dstDir ):
                    newName = osp.join(dstDir,
                            DSName.split('/')[-1].replace('_%s.rgbe'% imId, '.rgbe') )
                    os.system('cp %s %s' % (DSName, newName ) )
                    os.system('rm %s' % DSName )


            DSNames = glob.glob(osp.join(outDir, 'imDSNoOcclu%d*.rgbe' % n ) )
            for DSName in DSNames:
                imId = DSName.split('/')[-1].split('_')[-1].split('.')[0]
                dstDir = osp.join(outDir, 'light_%s' % imId )
                if osp.isdir(dstDir ):
