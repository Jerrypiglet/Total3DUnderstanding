...
if __name__ == '__main__':
    ...
    parser.add_argument('--re', default=1200, type=int, help='the height of the image' )
    ...
    for k in range(opt.rs, min(opt.re, len(scenes ) ) ):
        ...
        maskNames = glob.glob(osp.join(maskDir, 'imcadmatobj_*.dat' ) )
        if len(maskNames ) == 0:
            assert(False )
        ...
        for n in range(0, len(lightList ) ):
            ...
            for m in range(1, camNum + 1 ):
                ...
                maskName = lightMaskName.replace(outDir, \
                        maskDir ).replace('light%d' % n, 'im').replace('mask', 'cadmatobj').replace('.png', '.dat')
                assert(osp.isfile(maskName ) )
                with open(maskName, 'rb') as fIn:
                    hBuffer = fIn.read(4)
                    height = struct.unpack('i', hBuffer)[0]
                    wBuffer = fIn.read(4)
                    width = struct.unpack('i', wBuffer )[0]
                    mBuffer = fIn.read(width * height * 4 * 3 )
                    mask = np.array(
                        struct.unpack('i' * 3 * height * width,
                                      mBuffer), dtype = np.int32
                    )
                    mask = mask.reshape(height, width, 3)[:, :, 0:1]
                mask = np.concatenate([mask, mask, mask], axis=-1 )

                lightMask = cv2.imread(lightMaskName )

                if isWindow:
                    lightMask = np.logical_and(mask == 0, lightMask > 250 )
                    lightMask = lightMask.astype(np.uint8 ) * 255
                else:
                    lampMask1 = np.logical_and(mask >= 2666, mask <= 2720 )
                    lampMask2 = np.logical_and(mask >= 2558, mask <= 2561 )
                    lampMask = np.logical_or(lampMask1, lampMask2 )
                    lightMask = np.logical_and(lampMask, lightMask > 250 )
                    lightMask = lightMask.astype(np.uint8 ) * 255
